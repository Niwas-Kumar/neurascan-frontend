        ...
        <div>
              ...
              <div key={s} style={{ flex: 1, display: 'flex', alignItems: 'center' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, flex: 1 }}>
                  <div style={{
                    ...
                    background: step > i ? 'var(--violet)' : step === i ? 'var(--violet-dim)' : 'var(--bg-elevated)',
                    border: `2px solid ${step >= i ? 'var(--violet)' : 'var(--border)'}`,
                    color: step > i ? '#fff' : step === i ? 'var(--violet-soft)' : 'var(--text-muted)',
                    transition: 'all var(--duration)',
                    boxShadow: step === i ? '0 0 12px var(--violet-glow)' : 'none',
                  }}>
                    {step > i ? '✓' : i + 1}
                  </div>
                ...
                {i < STEPS.length - 1 && (
                  <div style={{ flex: 1, height: 2, background: step > i ? 'var(--violet)' : 'var(--border)', transition: 'background var(--duration-slow)', margin: '0 4px', marginBottom: 16 }} />
                )}
              </div>
            ))}
          </div>
          ...
          <div className="glass-panel" style={{ padding: '28px' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <select
                ...
                style={{
                  ...
                  background: 'var(--bg-input)', border: `1px solid ${studentId ? 'var(--violet)' : 'var(--border)'}`,
                  ...
                  boxShadow: studentId ? '0 0 0 3px rgba(124,58,237,0.12)' : 'none',
                }}
              >
            ...
            <motion.div
              {...getRootProps()}
              animate={{
                borderColor: isDragActive ? 'var(--violet)' : file ? 'var(--cyan)' : 'var(--border-strong)',
                background:  isDragActive ? 'rgba(124,58,237,0.08)' : file ? 'rgba(6,182,212,0.06)' : 'var(--bg-elevated)',
              }}
            ...
            >
              ...
              {uploading || analyzing ? (
                <div>
                  <div style={{ width: 52, height: 52, borderRadius: 13, background: 'var(--violet-dim)', border: '1px solid rgba(139,92,246,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                    <Zap size={24} color="var(--violet-soft)" />
                  </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', gap: 6 }}>
                      {[0,1,2].map(i => (
                        <motion.div key={i}
                          ...
                          style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--violet)' }}
                        />
                      ))}
                    </div>
                  )}
                ...
                <div>
                  <div style={{ width: 52, height: 52, borderRadius: 13, background: 'var(--cyan-dim)', border: '1px solid rgba(6,182,212,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
                    <FileImage size={24} color="var(--cyan-light)" />
                  </div>
                  <div style={{ fontWeight: 600, color: 'var(--cyan-light)', marginBottom: 4 }}>{file.name}</div>
                ...
                <div>
                  <div style={{ width: 52, height: 52, borderRadius: 13, background: isDragActive ? 'var(--violet-dim)' : 'var(--bg-card)', border: `1px solid ${isDragActive ? 'rgba(139,92,246,0.3)' : 'var(--border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', transition: 'all 0.15s' }}>
                    <Upload size={22} color={isDragActive ? 'var(--violet-soft)' : 'var(--text-muted)'} />
                  </div>
                  <p style={{ fontWeight: 600, marginBottom: 6, color: isDragActive ? 'var(--violet-soft)' : 'var(--text-primary)' }}>
